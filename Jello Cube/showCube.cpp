...
int pInclinedPlaneCalculated;
int nIntersections;
float cornerLocation[8][3];
int index;
...
void printBuffer(GLint size, GLfloat *buffer)
{
   ...
   while (count >=0) 
   {
      ...
      if (token == GL_PASS_THROUGH_TOKEN) {
         index = (int)buffer[size-count];
         count--;
      }
      else if (token == GL_POINT_TOKEN) 
	  {
		  cornerLocation[index][0] = buffer[size - count];
		  cornerLocation[index][1] = buffer[size - count + 1];
		  cornerLocation[index][2] = buffer[size - count + 2];
		  count -=8;
      }
   }
}
...
void showCylinder()
{
	...
	glTranslatef(cylinderPos.x,cylinderPos.y,cylinderPos.z);
	GLUquadricObj *quadratic, *sphereObj;
	quadratic = gluNewQuadric();
	sphereObj =  gluNewQuadric();
	//gluQuadricDrawStyle( quadratic, GLU_FILL );
	//gluQuadricOrientation(quadratic, GLU_INSIDE);
	gluCylinder(quadratic,cylinderBase,cylinderBase,cylinderHeight,nslices,nstacks); 
	glRotatef(180, 1,0,0);
	gluDisk(quadratic, 0.0f, cylinderBase, nslices, 1);
	glRotatef(180, 1,0,0);
	...
	//gluQuadricOrientation(sphereObj, GLU_INSIDE);
	glTranslatef(spherePos.x,spherePos.y,spherePos.z);
	gluSphere(sphereObj, sphereRadius, nslices, nstacks);
