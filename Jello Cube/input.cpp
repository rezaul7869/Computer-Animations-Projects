...
// Use the OPENGL feedback mechanism to get screen coordinates of pixels and do error checking
// Unused code for now. Used to implement dragging individual particles
void findMinMaxCorners(float* pminX, float* pmaxX,float* pminY, float* pmaxY,float* pminZ, float* pmaxZ)
{
	float minX= cornerLocation[0][0], maxX = cornerLocation[0][0], minY = cornerLocation[0][1];
	float maxY= cornerLocation[0][1], minZ = cornerLocation[0][2], maxZ = cornerLocation[0][2];
	for(int i = 1;i<8;i++)
	{
		if(cornerLocation[i][0] < minX) minX = cornerLocation[i][0];
		else if(cornerLocation[i][0] > maxX) maxX = cornerLocation[i][0];
		if(cornerLocation[i][1] < minY) minY = cornerLocation[i][1];
		else if(cornerLocation[i][1] > maxY) maxY = cornerLocation[i][1];
		if(cornerLocation[i][2] < minZ) minZ = cornerLocation[i][2];
		else if(cornerLocation[i][2] > maxZ) maxZ = cornerLocation[i][2];
	}
	*pminX = minX;
	*pminY = minY;
	*pminZ = minZ;
...
/*Get world coordinates from screen x and y coordinates*/
int GetWorldPosition(int x, int y, GLdouble* position)
{
    GLint viewport[4];
	...
	glReadPixels( winX, (int)(viewport[3] -winY), 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &winZ );	
	
	// Get world coordinates
    gluUnProject( winX, winY, winZ, modelview, projection, viewport, &position[0], &position[1], &position[2]);

	// If there are no surface under x and y coordinates glReadPixels returns far-z value, so gluUnProject returns invalid world coordinates
	// In that case use previous calculated winZ value
	if(position[0] > 2 || position[0] < -2 || position[1] > 2 || position[1] < -2 || position[2] > 2 || position[2] < -2)
	{
		if(prevWinZ != 0)
		winZ = prevWinZ;
		gluUnProject( winX, winY, winZ, modelview, projection, viewport, &position[0], &position[1], &position[2]);
...
void mouseMotionDrag(int x, int y)
{
  ...
  if (g_iRightMouseButton) // handle camera rotations
  {
    ...
    if (Theta<- pi / 2 + 0.01)
      Theta = -pi / 2 + 0.01;
    pInclinedPlaneCalculated =0;
    g_vMousePos[0] = x;
    g_vMousePos[1] = y;
  }

  /* This is to add external force with mouse drag with left button Currently disabled*/
  if (dispGeometry && g_iLeftMouseButton) 
  {
	  GLdouble prevPosition[3], nextPosition[3];
	  int result1 = GetWorldPosition(x,y,nextPosition); 
	  int result2 = GetWorldPosition(g_vMousePos[0],g_vMousePos[1],prevPosition);
	  if(result1 == 0 && result2 == 0){
		externalForce.x = (nextPosition[0] - prevPosition[0])*0.5;
		externalForce.y = (nextPosition[1] - prevPosition[1])*0.5;
		externalForce.z = (nextPosition[2] - prevPosition[2])*0.5;
		g_vMousePos[0] = x;
		g_vMousePos[1] = y;
	  }
  }
  ...
  {
	  // No dragging. reset external force
